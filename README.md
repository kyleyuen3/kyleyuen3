👋 Hi, I’m Kyle Yuen

Computer Science Graduate — Software Engineering · Systems · AI/ML

I’m a CS graduate from San Francisco State University passionate about software engineering, system administration, and AI/ML. I enjoy solving real problems, building clean user interfaces, and constantly learning new technologies that help people.

I’m currently seeking full-time roles in Software Engineering, Systems/IT, or AI/ML.

### 🛠 Tech Stack
- Languages: Java, Python, C++, JavaScript
- Web: React, Node.js, Express, HTML, CSS
- Databases: MySQL
- Tools: Linux, Git, GitHub, VS Code

👀 I’m interested in
- 🤖 Artificial Intelligence & Machine Learning (NLP, DistilBERT, language models)
- 🧠 Algorithms & Complexity (poly-reductions, Turing machines, automata)
- 🔧 Full-Stack Development (React, Node.js, MySQL)
- 🎮 Game Development & Web Projects (Canvas games, interactive sites)
- 📊 Data Science (feature engineering, statistical modeling, visualization)

🌱 I’m currently learning
- ⚛️ React and modern front-end development (JSX, components, hooks)
- 🖥️ IT & systems fundamentals for entry-level SysAdmin roles
- 📚 Brushing up on Java, C++, and Python
- 📂 Organizing and uploading my past university projects to GitHub

💼 Projects I’m proud of
- 🐊 Gator Market — Full-stack marketplace for students (React + Node + MySQL)
- 📈 Day Trading AI — ML agents analyzing market patterns
- 📚 N-gram Language Model Demo — Built for a 3-minute NLP presentation

📫 How to reach me
- 📧 Email: kyleyuen3@gmail.com
- 💼 LinkedIn: https://www.linkedin.com/in/kyleyuen
- 🖥️ Portfolio Website: https://portfolio-lilac-iota-88.vercel.app/

😄 Pronouns
- He/Him

⚡ Fun fact
- I’m a huge basketball fan, love building tech projects late at night, and I always challenge myself to learn something new every week.
